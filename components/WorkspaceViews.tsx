...
export default function WorkspaceViews({ view, leads, onSelectLead }: WorkspaceViewsProps) {
  ...
  if (view === 'activities') return <main className="mx-auto max-w-[1400px] p-6"><h1 className="mb-2 text-xl font-semibold text-[#26343C]">{heading}</h1><p className="mb-6 text-sm text-[#71808A]">Every recorded touch across the pipeline.</p><div className="rounded-xl border border-[#E1E8EC] bg-white divide-y divide-[#EDF1F3]">{activities.slice(0, 30).map(({ lead, activity }) => <button key={activity.id} onClick={() => onSelectLead(lead)} className="flex w-full items-start gap-3 px-4 py-3 text-left hover:bg-[#F8FAFB]"><div className="mt-1 h-7 w-7 rounded-lg bg-[#EAF3F6] flex items-center justify-center text-[#2B7393]"><ClipboardList className="h-3.5 w-3.5" /></div><div className="flex-1"><p className="text-sm text-[#33434C]"><span className="font-semibold">{lead.clinicName}</span> {activity.description}</p><p className="mt-1 text-xs text-[#8A979E]">{activity.type} · {formatDate(activity.timestamp)} · {activity.author}</p></div></button>)}</div></main>;

  if (view === 'accounts') return <main className="mx-auto max-w-[1400px] p-6"><h1 className="mb-2 text-xl font-semibold text-[#26343C]">{heading}</h1><p className="mb-6 text-sm text-[#71808A]">Clinic records and current relationship status.</p><LeadRows leads={leads} onSelectLead={onSelectLead} empty="No clinic accounts yet." /></main>;

  if (view === 'reports') return <main className="mx-auto max-w-[1400px] p-6"><h1 className="mb-2 text-xl font-semibold text-[#26343C]">{heading}</h1><p className="mb-6 text-sm text-[#71808A]">Operational summaries based on the current clinic records.</p><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Stat label="Total clinics" value={leads.length} icon={UsersRound} /><Stat label="Completed touches" value={leads.filter(l => l.outreachCompleted).length} icon={CheckCircle2} /><Stat label="Open opportunities" value={opportunities.filter(l => l.stage !== 'Prospect Closed').length} icon={FileBarChart} /><Stat label="Data issues" value={dataIssues.length} icon={Database} /></div></main>;

  if (view === 'data-quality') return <main className="mx-auto max-w-[1400px] p-6"><h1 className="mb-2 text-xl font-semibold text-[#26343C]">{heading}</h1><p className="mb-6 text-sm text-[#71808A]">Records that need cleanup before they are ready for outreach.</p><LeadRows leads={dataIssues} onSelectLead={onSelectLead} empty="All clinic records have the core fields." /></main>;

  const rows = view === 'followups' ? due : view === 'opportunities' ? opportunities : view === 'today' ? [...due, ...stale.filter(l => !due.some(d => d.id === l.id))] : due;
  return <main className="mx-auto max-w-[1400px] p-6"><div className="mb-6 flex items-end justify-between"><div><h1 className="text-xl font-semibold text-[#26343C]">{heading}</h1><p className="mt-1 text-sm text-[#71808A]">{view === 'today' ? 'The work that deserves attention first.' : view === 'opportunities' ? 'Active conversations and accounts moving toward a decision.' : 'Prioritized work from next-action dates and follow-up status.'}</p></div><CalendarClock className="h-5 w-5 text-[#2B7393]" strokeWidth={1.7} /></div><div className={`mb-6 grid gap-3 ${view === 'today' ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}><Stat label="Needs attention" value={due.length} icon={CircleAlert} /><Stat label="Stale over 7 days" value={stale.length} icon={CalendarClock} /><Stat label="Completed touches" value={leads.filter(l => l.outreachCompleted).length} icon={CheckCircle2} />{view === 'today' && <Stat label="Letters sent today" value={lettersSentToday} icon={Mail} />}</div><LeadRows leads={rows} onSelectLead={onSelectLead} empty={view === 'opportunities' ? 'No opportunities have reached an active conversation stage.' : 'Nothing needs attention right now.'} /></main>;
}
